...
各仓库的许可证相互独立，以该仓库的 `LICENSE`、`LICENSING.md` 和路径级许可证声明为准。没有这些明确声明时，不得从组织默认政策或其他仓库推定已经获得开源许可。e-dialect 有权授权的原创应用与服务代码默认采用 `AGPL-3.0-only`；第三方代码和单独治理的资产继续适用各自条款。

数据、语料、词典内容、录音、声音样本、模型、个人数据和品牌不因存放在代码仓库或
签署代码 CLA 而自动获得许可；分类和独立授权要求见
[ASSET_AND_DATA_POLICY.md](./ASSET_AND_DATA_POLICY.md)。

> 普通 CLA 流程主要用于代码及相应的软件文档。如果贡献包含方言语料、词典内容、
> PDF 或扫描材料、用户录音、声音样本、个人数据、模型权重或其他独立资产，请不要
> 直接通过普通代码 PR 提交资产本体。请先创建 Issue 或联系维护者，说明来源和拟用
> 途；不要在 Issue 中直接上传版权、隐私或授权状态不确定的材料，并等待维护者确认
> 适用的资产协议或授权路径。欢迎先提交公开来源链接、来源线索和处理这些材料的代码。

### 0.2 贡献者许可协议（CLA）

向 e-dialect 提交贡献不会转让你的版权。为了保持清晰、可审计的权利链，持续发布开源版本，并在权利链允许时提供替代许可，我们要求适用仓库的贡献者显式签署贡献者许可协议（CLA）。

#### 个人贡献者（ICLA）

如果你以个人身份贡献代码，且你的贡献不涉及雇主的知识产权，请阅读并显式接受：

- [个人贡献者许可协议 (ICLA)](./ICLA.md)

首次向接入 CLA 的仓库贡献时，自动化检查会引导你显式接受当前 ICLA 版本。同一版本通常只需签署一次。创建 Pull Request、提交 Commit 或勾选 PR 模板本身都不构成签署。
GitHub 用户名从签署时经 GitHub 验证的身份记录取得，无需重复手填。
签署数据的收集、用途、访问和请求渠道见 [CLA_PRIVACY.md](./CLA_PRIVACY.md)。

#### 企业贡献者（CCLA）

如果贡献属于雇主、公司或其他机构，请不要用个人 ICLA 代替企业授权。你需要：
...
AGPL 允许包括商业场景在内的使用，但使用者须遵守其全部条款。无法或不希望遵守 AGPL 条款的组织，可以联系对应项目维护者了解替代商业许可。替代许可是否可提供以及范围如何，取决于具体仓库的许可证、CLA、合同和可审计权利链；在历史贡献补签完成前，完整商业重许可能力仍标记为 `PARTIAL`。
